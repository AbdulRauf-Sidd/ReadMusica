...
class Staff {
	...
	setNotes(theNotes) {
		...
		if (dataType === "array"){
			...
			for(var i = 0; i < numberOfNotes; i++){
				...
				newElement.setAttribute("class","note1");
	...
	setNotesWithDistance(theNotes, number) {
		this.notes = theNotes;
		var thisElm = this.id;
		...
		if (dataType === "array"){
			...
			// remove current notes
			const removeElements = (elms) => elms.forEach(el => el.remove());
			removeElements( document.querySelectorAll("."+group) );
			
			
			numberOfNotes = this.notes.length;
			if (numberOfNotes > 3) {
				throw "setNotesWithDistances() must receive an array of length upto 3."
			}
			var num;
			let trans;
			// for each array element....
			for(var i = 0; i < numberOfNotes; i++){
				...
				newElement.id = 'note'+num;
				newElement.style.transform = `translateX(${trans})`;
				newElement.innerHTML = "a";
				//newElement.style.display = "block";
				svg.appendChild(newGroup);
				newGroup.appendChild(newElement);
